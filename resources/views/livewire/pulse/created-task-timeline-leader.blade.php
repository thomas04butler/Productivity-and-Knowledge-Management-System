<x-pulse::card :cols="$cols" :rows="$rows" :class="$class">
    <div>
        <x-pulse::card-header
            name="Tasks Created"
            title=""
            details="Number of tasks created in a day"
        >
        ...
        <x-pulse::scroll :expand="$expand">
            <div class="grid gap-3 mx-px mb-px">
                <div wire:key="requests-graph" >
                    <div class="mt-3 relative" x-data='taskCreationTimeChart({ sampleRate: 1 })'>

                        <div x-show="Object.keys(request.tasks_created ?? {}).length > 1" class="absolute -left-px -top-2 max-w-fit h-4 flex items-center px-1 text-xs leading-none text-white font-bold bg-purple-500 rounded after:[--triangle-size:4px] after:border-l-purple-500 after:absolute after:right-[calc(-1*var(--triangle-size))] after:top-[calc(50%-var(--triangle-size))] after:border-t-[length:var(--triangle-size)] after:border-b-[length:var(--triangle-size)] after:border-l-[length:var(--triangle-size)] after:border-transparent">
                            <span x-text="highest_val"</span>
                        </div>
                        <div wire:ignore class="" >

                            <template x-if="!(Object.keys(request.tasks_created ?? {}).length > 1)">
                                <x-pulse::no-results />
                            </template>
                            <div class="h-52" wire:ignore x-show="Object.keys(request.tasks_created ?? {}).length > 1">
                                <canvas x-ref="canvas"
...
@script
<script>
    Alpine.store('created_period', 720)

    Alpine.data('taskCreationTimeChart', (data) => ({
        ...
        async fetchData(){
            ...
            let date = new Date();
            date.setHours(date.getHours() - $store.created_period);
            ...
            let formatted = {};
            Object.keys(query).forEach(date => {
                const filteredTasks = query[date].filter(task => task.project.user_id === {{auth()->user()->id}});
                formatted[date] = filteredTasks.length;
            });

            this.request = { tasks_created: formatted };
        },
        ...
        async init() {
                        ...
                        datasets: [
                            {
                                ...
                                data: this.scale(request.tasks_created),
                                pointHitRadius: 10,
                                pointStyle: false,
                                tension: 0.2,
                                spanGaps: false,
            ...
            $watch('$store.created_period', async (value) => {
                ...
                chart.data.datasets[0].data = this.scale(request.tasks_created);
                chart.update()
            })
