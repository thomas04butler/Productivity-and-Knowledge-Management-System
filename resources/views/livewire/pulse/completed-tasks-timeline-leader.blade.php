    ...
    Alpine.data('taskCompletionTimeChart', (data) => ({
        ...
        async fetchData(){
            ...
            const params = {
                ...
                "order": "asc",
                "where[0][0]": "completed_at",
                "where[0][1]": ">",
                "where[0][2]": date.toISOString(),
                "page": "1",
                "perPage": "1000",
            ...
            let formatted = {};
            Object.entries(query).forEach(([date, items]) => {
                const filteredItems = items.filter(item => item.project.user_id === {{auth()->user()->id}});

                if (filteredItems.length > 0) {
                    const lengths = filteredItems.map(e => {
                        const date1 = new Date(e.completed_at);
                        const date2 = new Date(e.created_at);
                        const diffTime = Math.abs(date2 - date1);
                        const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24));
                        return diffDays;
                    }).reduce((a, b) => a + b, 0) / filteredItems.length;
